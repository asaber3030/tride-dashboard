...
export const DropzoneContent = ({ children, className }: DropzoneContentProps) => {
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center", className)}>
      ...
      <p className='my-2 w-full truncate font-medium text-sm'>{src.length > maxLabelItems ? `${new Intl.ListFormat("en").format(src.slice(0, maxLabelItems).map((file) => file.name))} and ${src.length - maxLabelItems} more` : new Intl.ListFormat("en").format(src.map((file) => file.name))}</p>
